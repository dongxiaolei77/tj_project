...
insert into dw_tmp.tmp_dwd_cust_biz_status_info_busi_type( cert_no
                                                         , busi_type)
select t2.id_no
     , concat(t3.value, '类')
from (
...
, busi_pl)
select t1.cert_no
     , group_concat(distinct case when t2.proj_origin = '05' then t3.busi_type else t4.ind_type_lv1_value end separator
                    '/')                                          as busi_type
     , group_concat(distinct t5.ind_type_lv2_value separator '/') as busi_class
from dw_base.dwd_guar_info_all t
...
truncate table dw_base.dwd_cust_biz_status_info;
commit;
...
from (
    ...
    select cert_no
         ...
         , case
               when right(group_concat(distinct bank_mgr separator '/'), 1) = '/'
                   then left(group_concat(distinct bank_mgr separator '/'),
                             char_length(group_concat(distinct bank_mgr separator '/')) - 1)
               else group_concat(distinct bank_mgr separator '/')
        end                                                 as bank_mgr
    from (select * from dw_base.dwd_guar_info_all where item_stt = '已放款') t
    group by cert_no
) t4
         ...
         left join dw_base.dim_area_info t2
                   on t1.city_code = t2.area_cd
         left join dw_base.dim_area_info t3
                   on t1.country_code = t3.area_cd
                       and t1.city_name = t3.sup_area_name
         left join dw_base.dim_area_info t6
                   on concat(SUBSTRING(t1.city_code, 1, 3), '000') = t6.area_cd
;
commit;
