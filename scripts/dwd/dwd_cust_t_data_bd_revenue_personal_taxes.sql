...
-- 开发人   :  xueguangmin
-- 开发时间 ： 20220117
-- 目标表   ： dwd_cust_t_data_bd_revenue_personal_taxes 大数据局-税务局-个人欠税信息
-- 源表     ： ods_de_t_data_bd_revenue_personal_taxes  大数据局-税务局-个人欠税信息
				-- ods_de_t_msg_log 报文日志表
				-- ods_de_t_cust_info 客户信息表
				-- dwd_cust_info 客户基本信息表
-- 变更记录 ： 20220117:统一变动
			-- 20220324:新建临时表 tmp_dwd_cust_t_data_bd_revenue_personal_taxes_pertax
--             20220516 日志变量注释  xgm   
-- ---------------------------------------

-- set @etl_date='${v_sdate}';
-- set @time=now();
-- set @table_name='dwd_cust_t_data_bd_revenue_personal_taxes';
-- set @sorting=@sorting+1;
-- set @auto_increment_increment=1; 


drop table if exists dw_base.tmp_dwd_cust_t_data_bd_revenue_personal_taxes_pertax;
commit;
create table dw_base.tmp_dwd_cust_t_data_bd_revenue_personal_taxes_pertax
(
	cum_owe_tax_tms varchar(10) COMMENT '累计欠税期数',
	owe_tax_amt varchar(50) COMMENT '欠税金额',
...
commit;
insert into dw_base.tmp_dwd_cust_t_data_bd_revenue_personal_taxes_pertax
...
select 
	...
	t1.createDate,
	t1.dataId
from dw_nd.ods_de_t_data_bd_revenue_personal_taxes t1
...
-- 大数据局-税务局-个人欠税信息
-- truncate  table dw_base.dwd_cust_t_data_bd_revenue_personal_taxes;
delete from dw_base.dwd_cust_t_data_bd_revenue_personal_taxes
where date_format(createDate,'%Y%m%d') =  '${v_sdate}';
commit;

insert into dw_base.dwd_cust_t_data_bd_revenue_personal_taxes
	(
	cust_id,
	cust_name,
	cert_no,
...
select 
	...
	t1.createDate,
	t1.dataId
from dw_base.tmp_dwd_cust_t_data_bd_revenue_personal_taxes_pertax t1
-- dw_nd.ods_de_t_data_bd_revenue_personal_taxes t1
...
commit;
-- insert into dw_base.pub_etl_log -- values (@etl_date,@pro_name,@table_name,@sorting,concat('大数据局-税务局-个人欠税信息表加工完成,共插入',@rowcnt,'条'),@time,now());commit;
