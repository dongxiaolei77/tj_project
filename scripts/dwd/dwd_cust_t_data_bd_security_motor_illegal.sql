...
-- 开发人   :  xueguangmin
-- 开发时间 ： 20220117
-- 目标表   ： dwd_cust_t_data_bd_security_motor_illegal 大数据局-公安厅-机动车违法信息
-- 源表     ： ods_de_t_data_bd_security_motor_illegal  大数据局-公安厅-机动车违法信息
				-- ods_de_t_msg_log 报文日志表
				-- ods_de_t_cust_info 客户信息表
				-- dwd_cust_info 客户基本信息表
-- 变更记录 ： 20220117:统一变动
			-- 20220324:新建临时表 tmp_dwd_cust_t_data_bd_security_motor_illegal_moill
--             20220516 日志变量注释  xgm   
-- ---------------------------------------

-- set @etl_date='${v_sdate}';
-- set @time=now();
-- set @table_name='dwd_cust_t_data_bd_security_motor_illegal';
-- set @sorting=@sorting+1;
-- set @auto_increment_increment=1; 


drop table if exists dw_base.tmp_dwd_cust_t_data_bd_security_motor_illegal_moill;
commit;
create table dw_base.tmp_dwd_cust_t_data_bd_security_motor_illegal_moill(
	count varchar(50),
	amt varchar(5) ,
	wfzfs varchar(5),
	info text,
...
insert into dw_base.tmp_dwd_cust_t_data_bd_security_motor_illegal_moill
select 
	t1.count,
	t1.amt,
	t1.wfzfs,
	...
	t1.createDate,
	t1.dataId
from dw_nd.ods_de_t_data_bd_security_motor_illegal t1
where  createDate between date_format('${v_sdate}','%Y-%m-%d') 
	and date_format(date_add('${v_sdate}', interval 1 day),'%Y-%m-%d');
commit;

-- 大数据局-公安厅-机动车违法信息
delete from  dw_base.dwd_cust_t_data_bd_security_motor_illegal
where date_format(createDate,'%Y%m%d') =  '${v_sdate}';  
-- truncate  table dw_base.dwd_cust_t_data_bd_security_motor_illegal;
commit;

insert into dw_base.dwd_cust_t_data_bd_security_motor_illegal
	(cust_id,
	cust_name,
	cert_no,
	count,
...
select 
	...
	t1.createDate,
	t1.dataId
from dw_base.tmp_dwd_cust_t_data_bd_security_motor_illegal_moill t1
-- dw_nd.ods_de_t_data_bd_security_motor_illegal t1
-- dw_nd.ods_de_t_data_bd_security_motor_illegal t1
...
commit;
-- insert into dw_base.pub_etl_log -- values (@etl_date,@pro_name,@table_name,@sorting,concat('大数据局-公安厅-机动车违法信息加工完成,共插入',@rowcnt,'条'),@time,now());commit;
