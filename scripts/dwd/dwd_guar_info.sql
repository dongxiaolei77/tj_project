...
-- 新业务中台
-- 担保业务平台续支业务上线
drop table if exists dw_tmp.tmp_guar_info_econ_info_01;
commit;
...
from (
         select proj_dtl_no
              , case
                    when length(econ_class_cd) = '23' then substr(econ_class_cd, 18, 4)
...
commit;
-- 更新状态
update dw_base.dwd_guar_info_new_busi
set item_stt ='已否决'
where aprv_amt = 0
...
select day_id
     ...
     , is_circ_loan
     , case
           when loan_bank like '%农业银行%' then '农业银行'
...
(
  ...
  default charset = utf8mb4
  collate = utf8mb4_bin comment ='标准线上业务台账数据的国民经济分类/国家农担分类编码';
commit;
...
from (
         ...
         from (
                  select t.apply_code
                       , substring_index(substring_index(concat(national_economy_classify, ','), ',', 1), '/',
                                         -1)                                               as econ_code
                       , substring_index(concat(national_guarantee_classify, ','), ',', 1) as guar_code
                  from (
                           select apply_code,
                                  national_economy_classify,
                                  national_guarantee_classify,
...
-- DATE_FORMAT(date_sub(date(now()),interval 1 day),'%Y%m%d')      -- 数据日期
    '${v_sdate}'
     ...
     , null                                                  -- 是否循环贷
     , case
           when t1.loan_bank like '%农业银行%' then '农业银行'
     ...
     , case
           when t1.busi_status <> '43' and t1.push_after_loan = '1' then '已放款'
...
(
  ...
  default charset = utf8mb4
  collate = utf8mb4_bin comment ='担保业务系统的项目状态';
commit;
...
select code                  as guar_id       -- 业务编号
     , case
           when t.status = '10' then '00' -- 项目状态：00-提报中10-审批中20-待签约30-待出函40-待放款50-已放款97-已作废98-已终止99-已否决91-不受理90-已解保
...
(
  ...
  default charset = utf8mb4
  collate = utf8mb4_bin comment ='担保业务系统的项目最大续支期数';
commit;

insert into dw_tmp.tmp_dwd_guar_info_all_item_stt_maxterm( project_id
                                                         , max_term)
select project_id
...
(
  ...
  default charset = utf8mb4
  collate = utf8mb4_bin comment ='画像系统代偿数据对应的原业务ID';
commit;
...
set item_stt_code =
        case
            when item_stt_code = '00' then '提报中'
...
-- 码值临时表插入数据
insert into dw_base.tmp_ods_imp_econ_info(code,
                                          value)
select value,
       max(code) as code
...
insert into dw_base.tmp_ods_t_biz_proj_unguar_idt(project_id, -- 业务ID
                                                  unguar_date) -- 解保日期
select b.project_id,
       b.unguar_date
from (
...
insert into dw_base.tmp_comp_totl_date_time_tdt(proj_code, -- 业务ID
                                                approp_totl, -- 代偿金额
                                                approp_date, -- 代偿日期
                                                update_time) -- 代偿登记日期
...
-- 产业集群临时表插入数据
insert into dw_base.tmp_ind_clus_gid_code(code,
                                          value)
...
select area_cd                 as city_code,
       substr(area_name, 1, 2) as city_name -- mdy 20220524 wyx
...
-- DATE_FORMAT(date_sub(date(now()),interval 1 day),'%Y%m%d') -- 数据日期
    '${v_sdate}'
     ...
     , t11.code                                           -- 国民经济分类
     , case
           when data_source in ('诸葛祥雨数据', '道一云线上数据', '龙戈数据') and guar_prod = '农耕贷' then '9999'
           else t7.code
    end                                                   -- 产品
     ...
     , case
           when t1.item_stt in ('已解保', '已代偿') and t13.unguar_date is null then t1.loan_end_dt
     ...
     , case
           when t1.guar_id like '%SDAGWF%XZ%' then substr(t1.guar_id, 1, 15)
...
set t1.mgr_id=t2.mgr_id,
    t1.mgr_tel=t2.tel_no
where t1.mgr_name is not null
  and t1.mgr_id is null
;
commit;
...
insert into dw_base.tmp_ods_mgr_vd_nx_rz(code,
                                         mgr_id,
                                         mgr_name)
select t1.code,
       t1.creator,
...
from (
         ...
         from (
                  ...
                  where code like 'VD%'
                     or code like 'NX%'
                     or code like 'RZ%' -- 银行客户经理编号存储为银行系统编号 VD NX RZ
              ) a
         where a.rn = 1
     ) t1
...
-- 修改台账星型表数据 分支机构名称 20250528
update dw_base.dwd_guar_info_stat t1 -- 担保台账星型表
    left join dw_base.dwd_imp_area_branch t2
    on t1.country_code = t2.CITY_CODE_
set t1.branch_office = case
                           when t2.ROLE_CODE_ = 'NHDLBranch' then '宁河东丽办事处'
                           when t2.ROLE_CODE_ = 'JNBHBranch' then '津南滨海新区办事处'
                           when t2.ROLE_CODE_ = 'BCWQBranch' then '武清北辰办事处'
                           when t2.ROLE_CODE_ = 'XQJHBranch' then '西青静海办事处'
                           when t2.ROLE_CODE_ = 'JZBranch' then '蓟州办事处'
