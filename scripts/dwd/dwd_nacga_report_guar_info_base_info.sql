...
delete from dw_base.dwd_nacga_report_guar_info_base_info where day_id = '${v_sdate}';
commit;
...
where day_id = '${v_sdate}'and -- 保证责任失效日期不准确 
	(((b.GUR_STATE = 'GT' and lend_reg_dt <= 20241231) or date_format(c.created_time,'%Y%m%d') >= 20250101) 	-- 2025年1月1日在保  GT(在保)、ED（解保）、DFK（待放款）、ZZ（终止）
