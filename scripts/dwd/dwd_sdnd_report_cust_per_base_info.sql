...
-- 开发人   : WangYX
-- 开发时间 ：20240831
-- 目标表   ：dwd_sdnd_report_cust_per_base_info      个人客户基础信息
-- 源表     ：dw_base.dwd_guar_info_all               业务信息宽表--项目域
--           dw_base.dwd_guar_info_stat              业务信息宽表--项目域
--           dw_base.dwd_sdnd_report_biz_no_base     国担上报范围表
--           dw_nd.ods_crm_cust_certification_info   crm客户认证信息表  -- 删除
--           dw_nd.ods_crm_cust_per_info             个人客户基本信息表
--           dw_nd.ods_wxapp_cust_login_info         客户注册信息
--           dw_nd.ods_t_biz_project_main            主项目表（进件表）
--           dw_nd.ods_bizhall_form_entry            业务大厅业务申请表单数据表

-- 备注     ：
-- 变更记录 ：20240913 通讯地址、配偶信息、性别、证件有效期切源 wyx
--            20241201 脚本的统一变更，TDS转MySQL8.0 zhangfl
-- ----------------------------------------

-- 日增量加载
delete from dw_base.dwd_sdnd_report_cust_per_base_info where day_id = '${v_sdate}';
commit;

insert into dw_base.dwd_sdnd_report_cust_per_base_info
(
 day_id
...
select '${v_sdate}' as day_id
	...
	,t1.cert_no	                       -- 证件号码
	,coalesce(t3.sex,case when substr(t1.cert_no,17,1) in ('1','3','5','7','9') then '1' else '2' end) as gender_cd -- 性别代码 /*从客户基本信息表出，取不到的根据证件号码判断*/
	,t3.cert_valid_start_dt            -- 证件有效期起始日期
	,t3.cert_valid_end_dt	           -- 证件有效期终止日期
	,case when length(trim(t1.tel_no)) <> 11 then coalesce(t3.tel,trim(t1.tel_no))
		  else trim(t1.tel_no)
	...
	,t2.coup_name	                   -- 配偶姓名
	,case when t2.coup_cert_no is not null then t2.coup_cert_typ_cd
		  else null
...
(
	...
	(
		select t1.cert_no
			...
			,t1.cust_name
			,t1.tel_no
			,row_number() over(partition by t1.cert_no order by t1.loan_reg_dt desc) rk /*业务宽表里取证件号最新放款登记日期对应数据*/
...
(
    ...
    ,coup_name
    ,coup_cert_typ_cd
    ,case when char_length(replace(coup_cert_no,' ',''))<>18                   -- mysql端是字节长度,这里保持一致,采用字节长度
	...
	(
		...
		,coalesce(t3.spouse_tel_no,t4.spouse_tel_no)                       as coup_tel_no   -- 配偶手机号 /优先取业务系统，缺失的用业务大厅表单数据补充/
		,row_number() over(partition by t2.cert_no order by t2.loan_reg_dt desc) as rk      /*业务宽表里取证件号最新放款登记日期对应数据*/
		...
		(
			...
			from
			(
				select	t1.id, t1.code, t1.cust_addr, t1.marital_status, t1.spouse_name, t1.spouse_id_no, t1.spouse_mobile
		...
		(
			select	t1.busi_code
					,replace(coalesce(
					json_extract(convert(json_val using utf8),'$.custPerInfoDTO.liveAddress')
					,json_extract(convert(json_val using utf8),'$.custPerInfoVO.custPerInfo.liveAddress')
					,json_extract(convert(json_val using utf8),'$.liveAddress')
					),'undefined','') as cust_address
					,coalesce(
					json_extract(convert(json_val using utf8),'$.custPerInfoDTO.marriageStatus')
					,json_extract(convert(json_val using utf8),'$.custPerInfoVO.custPerInfo.marriageStatus')
					,json_extract(convert(json_val using utf8),'$.marriageStatus')
					) as mgr_stt
					,coalesce(
					json_extract(convert(json_val using utf8),'$.custPerInfoDTO.spouseName')
					,json_extract(convert(json_val using utf8),'$.custPerInfoVO.custPerInfo.spouseName')
					,json_extract(convert(json_val using utf8),'$.spouseName')
					) as spouse_name
					,coalesce(
					json_extract(convert(json_val using utf8),'$.custPerInfoDTO.spouseIdNo')
					,json_extract(convert(json_val using utf8),'$.custPerInfoVO.custPerInfo.spouseIdNo')
					,json_extract(convert(json_val using utf8),'$.spouseIdNo')
					) as spouse_cert_no
					,coalesce(
					json_extract(convert(json_val using utf8),'$.custPerInfoDTO.spouseMobileNo')
					,regexp_replace(json_extract(convert(json_val using utf8),'$.adultChildrenList.spouseMobileNo'),'\\["|\\"]','')
					,json_extract(convert(json_val using utf8),'$.spouseMobileNo')
					) as spouse_tel_no
			from
			(
				select	t1.busi_code, t1.json_val, row_number()over(partition by t1.busi_code order by t1.update_time desc) rn
...
(
	...
	(
		...
		(
			...
			from
			(
				select	t1.id_no, t1.sex, t1.cust_code
