...
)ENGINE=InnoDB  DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin comment='国担上报-项目基础信息临时表-存业务的反担保措施';
commit;
...
(
	...
	(
		...
		(
			select	t1.project_id, t1.create_time, t1.reply_counter_guar_meas
					,row_number()over(partition by t1.project_id, t1.member_id, t1.`count` order by t1.update_time desc) rn
		...
		(
			select	t1.project_id, t1.create_time, t1.reply_counter_guar_meas
					,row_number()over(partition by t1.project_id, t1.creator order by t1.update_time desc) rn
...
-- 日增量加载
delete from dw_base.dwd_tjnd_report_proj_base_info where day_id = '${v_sdate}' ;
commit;
...
select	distinct '${v_sdate}'                as day_id
	...
	,case when t1.econ_class regexp "谷物|稻谷|小麦|玉米|杂粮" then '010000'
		...
		when t1.econ_class regexp "农业专业及辅助性活动|机械|防治|服务|灌溉|粪污" OR t1.guar_class = '农资、农机、农技等农业社会化服务' then '360500'
	...
	,case when t1.guar_class regexp'粮食种植|重要特色农产品种植' then '010000'
	...
	,case when t3.is_first_guar = '0' and t3.is_xz = '0' then '1'
		else '0'
	end as is_frst_guar	                                             -- 是否首担 /*首次担保+首次放款*/
	,case when t1.guar_amt between 10 and 300 then '1'
		else '0'
	end as is_policy_biz	                                         -- 是否政策性业务 /*政策性业务，10-300*/
	,'0' as is_guar_upon_loan_apply	                                 -- 是否见贷即保
	,'0' as is_comp_limit_rate	                                     -- 是否限率代偿
	,null as limit_rate	                                             -- 限率
	...
	,case when t17.proj_no_prov is not null and t1.item_stt = '已放款' then '03' /*关联逾期记录表，判断状态是否为逾期*/
	...
	,case when t19.counter_guar_meas like'%,%' then '9999' /*多个反担保方式代码的，置成组合*/
		when regexp_replace(regexp_replace(t19.counter_guar_meas,'\\[|\\]|null|\\"|',''),'^(,*+)','') = '00' THEN '01' /*剔除不规则数据*/
		else coalesce(if(length(regexp_replace(regexp_replace(t19.counter_guar_meas,'\\[|\\]|null|\\"|',''),'^(,*+)','')) = 0,'01',regexp_replace(regexp_replace(t19.counter_guar_meas,'\\[|\\]|null|\\"|',''),'^(,*+)','')),'01') /*空值置成01*/
	end as count_guar_cls_cd                                         -- 反担保方式代码
	,case when t1.data_source = '标准线上业务台账' then coalesce(date_format(t1.loan_begin_dt, '%Y-%m-%d'),date_format(t1.loan_reg_dt, '%Y-%m-%d'))
	...
	,case when left(t1.guar_id, 4) in ('ZZXZ','BHJC') then least(date_format(coalesce(t1.accept_dt,'99991231'), '%Y-%m-%d'),date_format(coalesce(t1.loan_begin_dt,'99991231'), '%Y-%m-%d')) /*自主续支存在续支补录历史担保年度数据，取非空最小值*/
		else date_format(t1.accept_dt, '%Y-%m-%d')
	end as apply_dt	                                                 -- 申请日期 /*mdy 20250107*/
	,case when coalesce(t1.appl_amt,t5.bank_credit_limit) = 0 or coalesce(t1.appl_amt,t5.bank_credit_limit) is null then t1.loan_amt
		else coalesce(t1.appl_amt,t5.bank_credit_limit)
	end as apply_amt	                                             -- 项目申请金额 /*优先取申请金额，用银行授信额度补充，如果为0或空值，取合同金额*/
	,case when left(t1.guar_id, 2) = 'XZ' and t1.aprv_term is not null then t1.aprv_term
	...
	,case when left(t1.guar_id, 4) in ('ZZXZ','BHJC') then date_format(t1.loan_begin_dt, '%Y-%m-%d') /*自主续支，取担保年度开始日*/
		else date_format(t1.aprv_dt, '%Y-%m-%d') /*其余取批复日期mdy 20250107*/
	end as aprv_dt	                                                 -- 农担批复日期
	,case when t1.data_source = '标准线上业务台账' and t1.loan_no is null then t5.credit_serial_no /*线上业务合同编号为空的，取授信业务编号*/
		else t1.loan_no
	end as loan_cont_no	                                             -- 借款合同编号
	,case when t1.data_source = '标准线上业务台账' then date_format(t1.loan_begin_dt, '%Y-%m-%d')
		else date_format(t7.jk_ctrct_start_date, '%Y-%m-%d')
	end as loan_begin_dt	                                         -- 借款合同生效日期    /*其余取借款合同开始日 mdy 20250107*/
	,case when t6.dept_name in ('农村商业银行','村镇银行') or (t1.guar_prod = '农耕e贷' and t6.bank_name = '农村商业银行') or t6.dept_id is null then t1.guar_id
		else t6.dept_id
	...
	,case when coalesce(t9.loan_type, t4.loan_type) = '1' then '1'   -- 1--自主循环贷(随借随还)
		else '0'
	end as is_self_renewal	                                         -- 是否为自主循环使用
	,null as loan_repay_typ	                                         -- 借款合同还款方式代码 main.loan_repay_type
	,case when t1.guar_prod = '赈灾贷' and coalesce(t1.guar_rate,t8.guar_rate)/100 = 0.03 then 0
	...
	,regexp_replace(trim(coalesce(t11.loan_notice_no, t5.loan_notice_no)),'\\(|\\（|\\)|\\）','') as gtee_agmt_no -- 保证合同编号 /*进件续支--放款通知书编号、自主续支--进件放款通知书、线上--电子签章*/
	,'03' as gtee_scp_cd	                                         -- 保证合同保证范围代码 /*固定值 03-本金利息及其他费用*/
	,case if(left(t1.guar_id, 2) = 'XZ' and t1.aprv_term is not null, t1.aprv_term, timestampdiff(month, t1.loan_begin_dt, t1.loan_end_dt))
		when 3 then '00'
		when 6 then '01'
		when 12 then '02'
		when 24 then '03'
	...
	,case when t1.data_source = '标准线上业务台账' then '02'
		else t4.guar_type
...
left join
(
	select	t1.id, t1.code as proj_no
			,t1.source, t1.loan_type, t1.guar_type
	from
	(
		select	t1.id, t1.code, t1.source, t1.loan_type, t1.guar_type
				,row_number()over(partition by t1.code order by t1.db_update_time desc, t1.update_time desc) rn
...
(
	...
	(
		...
		from
		(
			select	t1.biz_id, t1.signed_file_id, t1.status
...
(
	...
	(
		select	t1.project_id
				,t1.jk_ctrct_start_date
				,row_number()over(partition by t1.project_id order by t1.db_update_time desc, t1.update_time desc) rn
...
on if(t1.guar_id is not null and left(t1.guar_id,4) in( 'ZZXZ', 'BHJC'),t2.proj_no,t1.guar_id) = t19.guar_id
;