...
-- ----------------------------------------
-- 创建临时表查询数据
drop table if exists dw_tmp.tmp_dwd_tjnd_cust_auth_info;
create table dw_tmp.tmp_dwd_tjnd_cust_auth_info
select *
...
update dw_base.dwd_tjnd_report_cust_per_base_info t1 left join
    dw_tmp.tmp_dwd_tjnd_cust_auth_info t2 on t1.cert_no = t2.cert_no
...
-- 更新个人客户配偶信息
update dw_base.dwd_tjnd_report_cust_per_base_info t1 left join
    dw_tmp.tmp_dwd_tjnd_cust_auth_info t2 on t1.coup_cert_no = t2.cert_no
...
update dw_base.dwd_tjnd_report_cust_corp_rel_info t1 left join
    dw_tmp.tmp_dwd_tjnd_cust_auth_info t2 on t1.own_comp_cert_no = t2.cert_no
...
update dw_base.dwd_tjnd_report_cust_corp_base_info t1 left join
    dw_tmp.tmp_dwd_tjnd_cust_auth_info t2 on t1.corp_cert_no = t2.cert_no
set t1.is_info_auth       = if(t2.cert_no is null, 0, 1),             -- 是否取得信息查询授权
    t1.info_auth_start_dt = if(t2.cert_no is null, null, t2.sign_dt), -- 取得信息查询授权日期
    t1.info_auth_end_dt   = if(t2.cert_no is null, null, t2.end_dt)   -- 信息查询授权终止日期
where t1.day_id = '${v_sdate}';
...
-- 更新企业客户法人信息
update dw_base.dwd_tjnd_report_cust_corp_base_info t1 left join
    dw_tmp.tmp_dwd_tjnd_cust_auth_info t2 on t1.lgpr_cert_no = t2.cert_no
...
update dw_base.dwd_tjnd_report_proj_cntr_agmt_info t1 left join
    dw_tmp.tmp_dwd_tjnd_cust_auth_info t2 on t1.main_signer_cert_no = t2.cert_no
