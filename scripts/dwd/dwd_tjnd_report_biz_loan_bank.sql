...
select '${v_sdate}'             as day_id,
       ...
       t3.risk_shar_ratio       as bank_risk,
       100 - t3.risk_shar_ratio as tjnd_risk,
       null                     as gov_risk,
...
from (
         ...
         left join dw_nd.ods_t_sys_dept t2 -- 部门表
     ...
     (
         select t1.bank_id,
                t3.contract_id,
                t3.risk_shar_ratio
         from dw_nd.ods_org_manage_bank_info t1 -- 银行信息表
                  left join dw_nd.ods_org_manage_contract_info t2 -- 合同基础信息表
                            on t1.bank_id = t2.bank_id
                  left join dw_nd.ods_org_manage_comp_info t3 -- 分险比例详情表
                            on t2.id = t3.contract_id
     ) t3
         -- 祖籍列表包含银行表 或者 部门表id等于银行id
     on FIND_IN_SET(t3.bank_id, t2.ancestors) > 0 or t2.dept_id = t3.bank_id
         left join
     (
         select t1.dept_id,
                t2.序号,
