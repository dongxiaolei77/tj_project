...
--
-- 备注     ：天津迁移在保业务转进件,项目基础信息表取旧系统数据涉及字段: 项目类型代码、资金支持品种、资金支持环节、是否限率代偿、限率、项目申请期限、农担批复日期、保证合同编号、保证合同保证范围代码、保证合同保证期间代码、保证方式代码、
--                                                              保证责任生效日期、保证责任失效日期、代偿宽限期、项目所属行业代码（农担体系）、是否首担、申请日期、保证合同金额、签约金融机构（分支机构）
...
--          20250905 分支结构该从mian表取数 
--          20251016 修改政策性判断逻辑
--          20251017 修改项目申请期限逻辑,修改项目所属行业（省农担），原映射关系不对
--          20251029 逾期三个月未代偿余额（扣除银行分险）/逾期三个月利息以及其他费用金额余额（扣除银行分险）/首次代偿通知日期修改逻辑
-- ----------------------------------------
...
select '${v_sdate}'                                                   as day_id
     ...
     , case
           when a.CERT_TYPE = '1' then '01'
           when a.CERT_TYPE = '2' then '02'
    end                                                               as cert_typ_cd                     -- 项目主体证件类型代码，待映射
     , a.ID_NUMBER                                                    as cust_cert_no                    -- 项目主体证件号码
     , c.ITEM_SOURCE                                                  as proj_rsue_cd                    -- 项目来源代码，待补充
     , c.ITEM_TYPE                                                    as proj_typ_cd                     -- 项目类型代码，待补充
     , case
           when a.guarantee_code = 'TJRD-2021-5Z88-9694'
               then JSON_UNQUOTE(JSON_EXTRACT(a.area, '$[2]')) -- [这笔项目做单独处理：这个业务是河北省唐山市路南区， 数组[1]是唐山市对不上就特殊处理了]
           else JSON_UNQUOTE(JSON_EXTRACT(a.area, '$[1]')) end        as proj_blogto_area_cd             -- 项目所属区域代码
     , case
           when a.enter_code = 'NHDLBranch' then '1200000001'
           when a.enter_code = 'JNBHBranch' then '1200000002'
           when a.enter_code = 'BCWQBranch' then '1200000003'
           when a.enter_code = 'XQJHBranch' then '1200000004'
           when a.enter_code = 'JZBranch' then '1200000005'
           when a.enter_code = 'BDBranch' then '1200000006'
    end                                                               as proj_blogto_org_no              -- 项目所属机构，待映射
     , a.MAINBODY_TYPE_CORP                                           as proj_main_typ_cd                -- 项目主体类型代码，待映射
     , a.INDUSTRY_CATEGORY_COMPANY                                    as proj_blog_busntp_no_nacga       -- 项目所属行业代码（农担体系），待映射
     , case
           when a.INDUSTRY_CATEGORY_COMPANY = '01' then '粮食种植'                     -- '02'
           when a.INDUSTRY_CATEGORY_COMPANY = '02' then '重要特色农产品种植'             -- '09'
           when a.INDUSTRY_CATEGORY_COMPANY = '03' then '生猪养殖'                     -- '01'
           when a.INDUSTRY_CATEGORY_COMPANY = '04' then '其他畜牧业'                   -- '08'
           when a.INDUSTRY_CATEGORY_COMPANY = '05' then '渔业生产'                     -- '02'
           when a.INDUSTRY_CATEGORY_COMPANY = '06' then '农田建设 非农项目'             -- '99'
           when a.INDUSTRY_CATEGORY_COMPANY = '07' then '农产品流通'                   -- '10'
           when a.INDUSTRY_CATEGORY_COMPANY = '08' then '农产品初加工'                 -- '06'
           when a.INDUSTRY_CATEGORY_COMPANY = '09' then '农业新业态'                   -- '05'
           when a.INDUSTRY_CATEGORY_COMPANY = '10' then '其他农业项目'                 -- '03'
           when a.INDUSTRY_CATEGORY_COMPANY = '12' then '农资、农机、农技等农业社会化服务'  -- '04'
           when a.INDUSTRY_CATEGORY_COMPANY = '99' then '非农项目'                     -- '07'                        20251017
    end                                                               as proj_blog_busntp_prov_cd        -- 项目所属行业（省农担）
     , JSON_UNQUOTE(JSON_EXTRACT(a.INDUSTRY_CATEGORY_NATION, '$[3]')) as proj_nation_econ_cd             -- 项目所属行业代码（国民经济分类），待映射
     , c.SUPPLY_TYPE                                                  as proj_fin_sup_typ_cd             -- 资金支持品种，待补充
     , c.SUPPLY_PHASE                                                 as proj_fin_sup_link_cd            -- 资金支持环节，待补充
     , a.PRODUCT_NAME                                                 as proj_prod_name                  -- 担保产品名称
...
#      , case when a.BUSI_MODE_NAME = '见贷即保' then '1' else '0' end    as is_guar_upon_loan_apply   -- 是否见贷即保
     ...
     , a.bank_cale_rate                                               as fin_org_risk_share_ratio        -- 金融机构分险比例
     , a.gov_cale_rate                                                as gov_risk_share_ratio            -- 政府分险比例
     , a.coop_cale_rate                                               as other_risk_share_ratio          -- 其他机构分险比例
     ...
     , case
           when t5.biz_id is not null and a.GUR_STATE = '50' then '02'
           when a.GUR_STATE = '50' then '01'
           when a.GUR_STATE = '90' then '04'
           when a.GUR_STATE = '93' then '05'
    end                                                               as proj_stt_cd                     -- 项目状态代码
     , case
           when a.COUNTER_GUR_METHOD like '%,%' then '9999' /*多个反担保方式代码的，置成组合*/
           when regexp_replace(regexp_replace(a.COUNTER_GUR_METHOD, '\\[|\\]|null|\\"|', ''), '^(,*+)', '') =
                '00' then '01' /*剔除不规则数据*/
           else coalesce(if(length(regexp_replace(
                   regexp_replace(a.COUNTER_GUR_METHOD, '\\[|\\]|null|\\"|', ''), '^(,*+)', '')) = 0, '01',
                            regexp_replace(regexp_replace(a.COUNTER_GUR_METHOD, '\\[|\\]|null|\\"|', ''),
                                           '^(,*+)', '')), '01') /*空值置成01*/
    end                                                               as count_guar_cls_cd               -- 反担保方式代码，待映射
     , date_format(a.lend_reg_dt, '%Y-%m-%d')                         as on_guared_dt                    -- 计入在保日期
     , a.GT_AMOUNT                                                    as proj_onguar_amt_totl            -- 项目在保余额
     , null                                                           as biz_scale                       -- 经营规模
     , null                                                           as biz_year                        -- 经营年限，非必填
     , null                                                           as biz_lcal_typ_cd                 -- 经营场地类型代码，非必填
     , a.BUSINESS_REVENUE / 10000                                     as opr_income                      -- 营业收入，非必填 需除以10000
     , a.OPERATION_LOAN / 10000                                       as opr_debts                       -- 经营性负债
     , null                                                           as create_jobs_num                 -- 带动就业人数，非必填
     , null                                                           as create_income_amt               -- 带动农民增收金额，非必填 需除以10000
     , a.OFFICE_ADDRESS                                               as biz_lcal_addr                   -- 经营详细地址
--     , date_format(a.apply_time, '%Y-%m-%d')                          as apply_dt                        -- 申请日期
     , t6.apply_dt                                                    as apply_dt                        -- 申请日期               [这里z_report 与 z_migrate 的数据不一样，前者做过处理，所以先用老数据]
     , a.APPLICATION_AMOUNT                                           as apply_amt                       -- 项目申请金额
     ...
     , t4.机构编码                                                        as loan_bank_no                    -- 签约金融机构代码，待映射
--     , a.FULL_BANK_NAME                                             as loan_bank_br_name               -- 签约金融机构（分支机构）
     , t6.full_bank_name                                              as loan_bank_br_name               -- 签约金融机构（分支机构）          [若项目已提交代偿补偿申请，该项目基础信息不可修改或删除（可以新增记录）]
     , a.LOAN_CONTRACT_AMOUNT                                         as loan_amt                        -- 借款合同额度
     ...
     , case
           when a.REPAYMENT_WAY = '01' then '11'
           when a.REPAYMENT_WAY = '02' then '07'
           when a.REPAYMENT_WAY = '03' then '02'
           when a.REPAYMENT_WAY = '04' then '01'
    end                                                               as loan_repay_typ                  -- 借款合同还款方式代码
     , a.GUARANTEE_TATE                                               as guar_fee_rate                   -- 担保费率
     , t4.机构编码                                                        as gtee_bank_no                    -- 保证合同金融机构代码，待映射
     ...
     , trim(d.GUARANTEE_END_DATE)                                     as gtee_expr_dt                    -- 保证责任失效日期，待补充
     , cast(a.COMPENSATION_PERIOD as signed)                          as comp_duran                      -- 代偿宽限期
     , null                                                           as comp_rmv_amt                    -- 项目核销金额，非必填
     , null                                                           as comp_rmv_dt                     -- 项目核销日期，非必填
     , d.GUARANTEE_CONTRACT_AMOUNT / 10000                            as gtee_cont_amt                   -- 保证合同金额，待补充 需除以10000
     ...
     , date_format(t2.FIRST_NOTICE_DATE, '%Y-%m-%d')                  as COMP_FST_COMP_NTC_DT            -- 首次代偿通知日期
from dw_base.dwd_tjnd_yw_guar_info_all_qy a -- 迁移业务宽表
         inner join dw_base.dwd_nacga_report_guar_info_base_info b -- 上报业务基础表
                    on a.guarantee_code = b.biz_no
         inner join dw_nd.ods_creditmid_v2_z_migrate_afg_business_infomation c
                    on a.id_business_information = c.id -- 业务信息
         inner join dw_nd.ods_creditmid_v2_z_migrate_afg_business_approval d -- 业务审批
                    on a.id_business_information = d.ID_BUSINESS_INFORMATION
         left join
     (
         select id_cfbiz_underwriting, FIRST_NOTICE_DATE
     ...
     on c.COOPERATIVE_BANK_ID = t3.dept_id
         left join
     (
         select t1.dept_id,
                t2.机构编码,
     ...
     ) t4 on FIND_IN_SET(t4.dept_id, t3.ancestors) > 0 or t3.dept_id = t4.dept_id
         left join
     (
         select bhecd.ID_CFBIZ_UNDERWRITING as biz_id
         from dw_nd.ods_creditmid_v2_bh_warning_tracking bhwt
                  inner join dw_nd.ods_creditmid_v2_z_migrate_bh_early_warning bhew
                             on bhwt.ID_WARING = bhew.ID
                  inner join dw_nd.ods_tjnd_yw_bh_early_contract_detail bhecd
                             on bhew.id = bhecd.id_early_warning
         where bhWt.OVER_TAG != 'BJ'
            or bhWt.OVER_TAG is null
     ) t5
     on c.id = t5.biz_id
         left join
     (
     ...
     ) t6 on a.guarantee_code collate utf8mb4_0900_ai_ci =
             t6.guarantee_code collate utf8mb4_0900_ai_ci -- [排序规则不一样，统一排序规则，否则关联不上]
         ...
         from (
                  ...
                  from (
                           select t1.id                      as                             project_id
                                , t1.create_time
                                , t1.apply_counter_guar_meas as                             reply_counter_guar_meas
                                , row_number()
                                   over (partition by t1.code order by t1.update_time desc) rn
                           from dw_nd.ods_t_biz_project_main t1-- 主进件表
                           where proj_origin = '02'
                       ) t1
                  where t1.rn = 1
                    and t1.reply_counter_guar_meas is not null
              ) t1
...
select distinct '${v_sdate}'                                                                      as day_id
              ...
              , if(t1.guar_id like 'TJ%', t21.ITEM_TYPE,
                   case
    ...
    )                                                                                             as proj_typ_cd                     -- 项目类型代码 /*通过业务编号判断，风险化解业务类型为风险续保；其他业务通过“是否首保”判断是新增或续保*/
...
--              , t18.table_no_nacga                                                                as proj_blogto_org_no              -- 项目所属机构 /*上报系统映射*/ -- mdy 20241209 wyx   
              , t4.branch                                                                         as proj_blogto_org_no              -- 项目所属机构  从main表里出      20250905
              , t0.cust_class_code                                                                as proj_main_typ_cd                -- 项目主体类型代码 /*关联国担上报个人、企业客户基本信息表,获取主体类型代码*/
              , case
                    when t1.guar_id like 'TJ%' then t0.guar_code -- 在保转进件的取原来的值   20250905
              ...
              , if(t1.guar_id like 'TJ%', t21.SUPPLY_TYPE,
                   case
    ...
    )                                                                                             as proj_fin_sup_typ_cd             -- 资金支持品种
              , if(t1.guar_id like 'TJ%', t21.SUPPLY_PHASE,
                   case
    ...
    )                                                                                             as proj_fin_sup_link_cd            -- 资金支持环节 /*根据业务提供映射规则转换：国担分类+经营主业*/
              , case when t1.guar_id not like 'TJ%' then t1.guar_prod end                         as proj_prod_name                  -- 担保产品名称
              , if(t1.guar_id like 'TJ%', t21.FIRST_GUARANTEE, -- [判断是否是在保转进件的项目]           20250905
              ...
              , if(t1.guar_id like 'TJ%', t21.IS_LIMITED_RATE_COMPENSATION,
                   t6.IS_COMP_LIMIT_RATE)                                                         as is_comp_limit_rate              -- 是否限率代偿
              , if(t1.guar_id like 'TJ%', if(t21.IS_LIMITED_RATE_COMPENSATION = 1, 0.03, null),
                   t6.LIMIT_RATE / 100)                                                           as limit_rate                      -- 限率
              , if(t1.guar_id like 'TJ%', t23.bank_org_rate, t6.bank_risk / 100)                  as fin_org_risk_share_ratio        -- 金融机构分险比例     -- [判断是否是在保转进件的项目]      20250905
              ...
              , if(t1.item_stt = '已放款',
                   if(t1.guar_id like 'TJ%', coalesce(t1.guar_amt, 0) - coalesce(t24.sum_repayment_principal, 0),
                       #                    if(t1.guar_id like 'TJ%',
#                       if(t22.guarantee_contract_amount / 10000 =
#                          case when t1.guar_id = 'TJRD-2021-5Z83-959W' then 47.950555 else t1.loan_amt end, -- [担保=借款]
#                          coalesce(t1.loan_amt, 0) - coalesce(t24.sum_repayment_principal, 0),
#                          coalesce(t1.guar_amt, 0) - coalesce(t24.sum_repayment_principal, 0)
#                           ),
                      t1.guar_amt), -- [特殊处理] 放款 - 解保
                   0)                                                                             as proj_onguar_amt_totl-- 项目在保余额 /*业务状态为“已放款”的合同金额*/
              , null                                                                              as biz_scale                       -- 经营规模
              ...
              , if(t1.guar_id like 'TJ%', t25.apply_dt,
                   case
              ...
              , if(t1.guar_id like 'TJ%', cast(t21.term as signed integer),
                   case
                       when left(t1.guar_id, 2) = 'XZ' and t1.aprv_term is not null
                           then t1.aprv_term
--                       else timestampdiff(month, t1.loan_begin_dt, t1.loan_end_dt)
                         else t1.aprv_term                                                       -- 20251017  修改项目申请期限逻辑
              ...
              , if(t1.guar_id like 'TJ%', date_format(t21.RECEIPT_DATE, '%Y-%m-%d'),
                   case
              ...
              , case
                    when t1.guar_id = 'TJRD-2021-5Z83-959W' then 47.950555
                    else t1.loan_amt end                                                          as loan_amt                        -- 借款合同额度   [临时改动]
              , t1.loan_term                                                                      as loan_period                     -- 借款合同期限
              , round(t1.loan_rate / 100, 4)                                                      as loan_cont_intr                  -- 借款合同利率
              , case
                    when t1.guar_id like 'TJ%' then t21.is_auto_revolving_loan --  [在保转进件的暂时取原数据的值]   20250905
                    when coalesce(t9.loan_type, t4.loan_type) = '1' then '1' -- 1--自主循环贷(随借随还)
                    else '0'
    end                                                                                           as is_self_renewal                 -- 是否为自主循环使用
              , if(t1.guar_id like 'TJ%',
                   case
                       when t4.loan_repay_type = '01' then '11'
                       when t4.loan_repay_type = '02' then '07'
                       when t4.loan_repay_type = '03' then '02'
                       when t4.loan_repay_type = '04' then '01'
                       end,
              ...
              , if(t1.guar_id like 'TJ%', t22.GUARANTY_CONTRACT_NO,
              ...
              , if(t1.guar_id like 'TJ%', t22.GUARANTEE_RANGE, '03')                              as gtee_scp_cd                     -- 保证合同保证范围代码 /*固定值 03-本金利息及其他费用*/
              , if(t1.guar_id like 'TJ%', t22.GUARANTEE_TIME_SLOT,
              ...
              , if(t1.guar_id like 'TJ%', t22.GUARANTEE_WAY,
                   case
                       when t1.data_source = '标准线上业务台账' then '02'
                       else t4.guar_type
                       end)                                                                       as gtee_mhd_cd                     -- 保证方式代码 /*线上全部为连带责任保证，其余从业务系统出*/
              , if(t1.guar_id like 'TJ%', t22.GUARANTEE_START_DATE,
                   case
                       when t1.data_source = '标准线上业务台账' then coalesce(date_format(t1.loan_begin_dt, '%Y-%m-%d'),
                                                                      date_format(t1.loan_reg_dt, '%Y-%m-%d'))
                       else date_format(t1.loan_reg_dt, '%Y-%m-%d')
                       end)                                                                       as gtee_eff_dt                     -- 保证责任生效日期
              , if(t1.guar_id like 'TJ%', trim(t22.GUARANTEE_END_DATE),
                   date_format(t1.loan_end_dt, '%Y-%m-%d'))                                       as gtee_expr_dt                    -- 保证责任失效日期
              , if(t1.guar_id like 'TJ%', cast(t23.COMPENSATION_PERIOD as signed), t6.comp_duran) as comp_duran                      -- 代偿宽限期
              , null                                                                              as comp_rmv_amt                    -- 项目核销金额
              , null                                                                              as comp_rmv_dt                     -- 项目核销日期
              , if(t1.guar_id like 'TJ%', t22.guarantee_contract_amount / 10000,
                   t1.guar_amt)                                                                   as gtee_cont_amt                   -- 保证合同金额         20250905
              ...
              , '1'                                                                               as is_direct_guar                  -- 是否直接承担担保责任
              , '00'                                                                              as is_coop_cd                      -- 项目合作情况代码
              , coalesce(t17.ovd_prin_rmv_bank_rk_seg_bal_3,0)  as ovd_prin_rmv_bank_rk_seg_bal_3   -- 逾期三个月未代偿余额（扣除银行分险）                            20251029
              , coalesce(t17.other_ovd_rmv_bank_rk_seg_bal_3,0) as other_ovd_rmv_bank_rk_seg_bal_3  -- 逾期三个月利息以及其他费用金额余额（扣除银行分险）                20251029
              , case when t17.proj_no_prov is not null and t29.is_receive = '1' then t29.receive_date      -- [如果保后任务新增字段里的是否收到代偿通知书字段为‘是’就取收到日期， 如果没有再用现在的取数逻辑]    20251029
			         else t28.comp_fst_comp_ntc_dt                                                          
					 end                                                                          as COMP_FST_COMP_NTC_DT            -- 首次代偿通知日期
from dw_base.dwd_guar_info_all t1 -- 业务信息宽表--项目域
     ...
     (
         select t1.id
              ...
              , t1.loan_repay_type
              , case
                    when t1.branch like '%宁河%' then '1200000001'
                    when t1.branch like '%津南%' then '1200000002'
                    when t1.branch like '%武清%' then '1200000003'
                    when t1.branch like '%静海%' then '1200000004'
                    when t1.branch like '%蓟州%' then '1200000005'
                    when t1.branch like '%宝坻%' then '1200000006'
     ...
     on t0.scheme_code = t14.dictionaries_code
         ...
         left join dw_base.dwd_tjnd_report_proj_rk_wrn_info t16 -- 国担上报-风险预警记录表
                   on t1.guar_id = t16.proj_no_prov
                       and t16.day_id = '${v_sdate}' and t16.dict_flag = '1'
         left join (
		            select proj_no_prov
					      ,if(date_add(ovd_dt,interval 3 month) < date_format('${v_sdate}','%Y-%m-%d'),ovd_prin_rmv_bank_rk_seg_bal,0)  as ovd_prin_rmv_bank_rk_seg_bal_3  -- 逾期三个月未代偿余额（扣除银行分险） 
                          ,if(date_add(ovd_dt,interval 3 month) < date_format('${v_sdate}','%Y-%m-%d'),other_ovd_rmv_bank_rk_seg_bal,0) as other_ovd_rmv_bank_rk_seg_bal_3 -- 逾期三个月利息以及其他费用金额余额（扣除银行分险）
					from (
		                    select proj_no_prov
					              ,sum(ovd_amt) as overdue_principal     --  逾期本金
			                      ,sum(other_ovd_amt) as overdue_int                 --  逾期利息（元）
					  	          ,sum(ovd_prin_rmv_bank_rk_seg_bal)  as ovd_prin_rmv_bank_rk_seg_bal      -- 逾期本金余额(扣除银行分险)                
					  	          ,sum(other_ovd_rmv_bank_rk_seg_bal) as other_ovd_rmv_bank_rk_seg_bal     -- 逾期利息以及其他费用金额余额(扣除银行分险)
			                      ,max(date_format(ovd_dt, '%Y-%m-%d')) as  ovd_dt             --  逾期日期
     ...
     (
         ...
         left join dw_nd.ods_creditmid_v2_z_migrate_afg_business_infomation t21 on t0.project_id = t21.id
         left join dw_nd.ods_creditmid_v2_z_migrate_afg_business_approval t22
                   on t21.id = t22.ID_BUSINESS_INFORMATION
         left join dw_nd.ods_tjnd_yw_base_cooperative_institution_agreement t23 -- 合作机构表（老逻辑底表）
                   on t21.related_agreement_id = t23.ID
         left join
     (
         select project_id, sum(repayment_principal) / 10000 as sum_repayment_principal
         from (
                  select *, row_number() over (partition by id order by db_update_time desc) rn
                  from dw_nd.ods_t_biz_proj_repayment_detail) t1
         where rn = 1
         group by project_id
     ) t24 on t0.project_id = t24.project_id
         left join
     (
         select guarantee_code                                    -- 业务编号
              , date_format(created_time, '%Y-%m-%d') as apply_dt -- 申请日期
     ...
     on t1.guar_id collate utf8mb4_0900_ai_ci = t25.guarantee_code collate utf8mb4_0900_ai_ci -- [排序规则不一样，统一排序规则，否则关联不上]
         left join
     ( -- 20250923 改动
     ...
     on t0.project_id = t26.id_business_information
         left join
     (
         select id_business_information
              , sum(receipt_amount) as loan_amt_fk
         from dw_nd.ods_creditmid_v2_z_migrate_afg_voucher_infomation
         where DELETE_FLAG = 1
         group by id_business_information
     ) t27 -- 放款凭证信息
     on t0.project_id = t27.id_business_information
    left join dw_base.dwd_tjnd_report_proj_comp_info t28 -- 代偿记录
    on t1.guar_id = t28.proj_no_prov and t28.day_id = '${v_sdate}'
	left join (
	           select project_id
			         ,is_receive   -- 是否收到履行代偿责任通知书  [1-是，0-否]
					 ,date_format(receive_date,'%Y-%m-%d') as receive_date -- 收到通知书日期
	...
	on t0.project_id = t29.project_id
;
commit;
-- 更新是否政策性业务代码
...
commit;
-- 更新是否政策性业务 -新增判断 base_customers_history.INDUSTRY_CATEGORY_COMPANY字段为99(非农项目)的业务, 就不是政策性业务             20250905
update dw_base.dwd_tjnd_report_proj_base_info
set is_policy_biz = '0'
where day_id = '${v_sdate}'
--  and proj_stt_cd in ('01', '02', '03')   -- 20251016 去掉在保限制，只要是非农项目，都为非政策性项目
  and proj_blog_busntp_no_nacga = '11';    
commit;
