...
--            zzy 20251027 代偿宽限期、主观原因修改取数逻辑，取消取固定值
-- ----------------------------------------
-- 日增量加载
delete
...
select '${v_sdate}'                                            as day_id
     ...
     , coalesce(substring_index(regexp_replace(t2.object_reason, '"|\\[|\\]', ''), ',', 1),
                '999999')                                           as subj_comp_rsn_cd        -- 项目代偿客观原因 -- 非结构化数据，提取首位编码，空值置为其他--   110
     , coalesce(substring_index(regexp_replace(t2.subject_reason,'"|\\[|\\]', ''), ',', 1),
	            '99')                                               as obj_comp_rsn_cd         -- 主观原因 -- 暂定为其他--    20251027   130
     , coalesce(t8.value, '无')                                as comp_rsn_desc           -- 项目代偿原因详述
     , coalesce(t9.comp_duran,'90')                            as comp_duran              -- 代偿宽限期                  20251027
     ...
     , 0                                                       as dict_flag                                          -- 20251127 底层表做了农担转码，这里不需要再转了
	 ,t2.appro_aply_overdue_amt                                as COMP_OVD_AMT            -- 代偿时逾期金额
from dw_base.dwd_guar_info_all t1
     ...
     (
         select distinct t1.proj_id
		               , t2.subjective_over_reason  as subject_reason         -- 主观风险成因
