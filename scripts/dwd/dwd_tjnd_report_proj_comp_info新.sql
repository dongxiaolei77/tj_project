...
select distinct '${v_sdate}' as day_id
	...
	,case when coalesce(t2.compt_aply_dt,'9999-12-31') < coalesce(t2.meet_date,'9999-12-31') then t2.compt_aply_dt
		  when coalesce(t2.compt_aply_dt,'9999-12-31') > coalesce(t2.meet_date,'9999-12-31') then t2.meet_date
	...
	,coalesce(substring_index(regexp_replace(t2.object_reason,'"\\[|\\]',''), ',', 1),110)      as subj_comp_rsn_cd	        -- 项目代偿客观原因 -- 非结构化数据，提取首位编码，空值置为其他-- 
	...
	,coalesce(t2.appro_aply_overdue_amt*t9.gov_risk/100,t9.gov_risk_amt) 						as rbl_rk_shr_amt_br_gov	-- 应收分险金额（地方政府） -- 截止拨付申请日逾期总额(元)*政府分险比例-- 
...
(
	...
	(
		select	t1.comp_id
				,t1.overdue_reason
				,t1.objective_over_reason
				,row_number()over(partition by t1.comp_id order by t1.db_update_time desc,t1.update_time desc) rn
...
(
	...
	(
		select	t1.ndc_id
				...
				,row_number()over(partition by t1.ndc_id order by t1.db_update_time desc,t1.update_time desc) rn
