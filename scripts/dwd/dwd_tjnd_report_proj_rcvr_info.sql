...
select distinct '${v_sdate}'                           as day_id
              ...
              , t4.cur_recovery * 10000                as rcvr_rcpt_amt
              , date_format(t4.entry_data, '%Y-%m-%d') as rcvr_rcpt_dt
              , 0                                      as dict_flag
from dw_base.dwd_nacga_report_guar_info_base_info t1 -- 国担上报范围表
         inner join dw_nd.ods_creditmid_v2_z_migrate_bh_recovery_tracking t3 -- 追偿跟踪表
                    on t3.ID_CFBIZ_UNDERWRITING = t1.biz_id
         inner join dw_nd.ods_creditmid_v2_z_migrate_bh_recovery_tracking_detail t4 -- 追偿跟踪详情表
 --                   on t4.id_recovery_tracking = t3.id
on ifnull(t4.ID_RECOVERY_TRACKING = t3.ID,t4.GUARANTEE_CODE = t3.RELATED_ITEM_NO)	
