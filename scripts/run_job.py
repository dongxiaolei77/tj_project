...
while not _conn_status and _conn_retries_count <= _max_retries_count:
    ...
    try:
        ...
        # 配置库
        # 测试
        # conn = pm.connect(host='10.82.136.12',port=3306,user='s_dba',password='Grp8nn5e8eAzwmv',charset='utf8',connect_timeout=5)
        # 生产 172.31.139.10
        conn = pm.connect(host='172.20.136.14', port=3306, user='p_dba', password='y7tPe3k%WHAvnEyz', charset='utf8',
                          connect_timeout=5)
        # _conn_status=True
        curs = conn.cursor()
        _conn_status = True
